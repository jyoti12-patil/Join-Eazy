...
export const GroupManager = () => {
  ...
  const handleAddMember = async (identifierToAdd) => {
    const ident = identifierToAdd || memberIdentifier;
    if (!ident.trim()) {
      toast.error('Please enter a student email or ID');
      return;
    }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      {group ? (
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-100">
                <div>
                  ...
                  <div className="flex items-center gap-4 mt-2 text-xs text-slate-500">
                    <span className="flex items-center gap-1">
                      ...
                      Code: <strong className="text-slate-700">{group.code}</strong>
                    </span>
                    <span>•</span>
                    <span>
                      Capacity: <strong>{group.members.length} / {group.maxMembers}</strong>
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-brand-50 hover:bg-brand-100 text-brand-700 border border-brand-200/60 text-xs font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 p-10 text-center shadow-xs max-w-2xl mx-auto space-y-6">
          <div className="w-16 h-16 bg-brand-50 text-brand-600 rounded-3xl flex items-center justify-center mx-auto shadow-inner">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1 max-w-md mx-auto">
              At JoinEazy, assignments are completed and tracked in student groups. Form a group now or ask a leader to invite you via your email (<span className="font-mono text-brand-600">{user?.email}</span>).
            </p>
          </div>
          ...
          <div className="pt-2 flex justify-center">
            <button
              onClick={() => setCreateModalOpen(true)}
              className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-brand-600 hover:bg-brand-700 text-white font-bold text-sm shadow-md hover:shadow-glow transition-all"
            >
            ...
            <div className="pt-6 border-t border-slate-100 text-left">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {allGroups.map((g) => (
                  <div
                    key={g.id}
                    className="p-3.5 rounded-2xl border border-slate-200/70 bg-slate-50/50 flex items-center justify-between"
                  >
                    <div>
        ...
        <form onSubmit={handleCreateGroup} className="space-y-4">
          ...
          <div className="pt-4 flex justify-end gap-3 border-t border-slate-100">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="px-5 py-2.5 bg-brand-600 hover:bg-brand-700 disabled:opacity-50 text-white text-sm font-bold rounded-xl shadow-md transition-all"
            >
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3.5 py-2.5 text-sm rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500"
              />
              <button
                type="button"
                disabled={submitting || !memberIdentifier.trim()}
                onClick={() => handleAddMember()}
                className="px-4 py-2.5 bg-brand-600 hover:bg-brand-700 disabled:opacity-50 text-white text-xs font-bold rounded-xl shadow-xs transition-all"
              >
            ...
            <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
              {availableStudents
                ...
                .map((stu) => (
                  <div
                    key={stu.id}
                    className="p-2.5 rounded-xl border border-slate-100 hover:border-brand-200 bg-slate-50/60 hover:bg-brand-50/50 flex items-center justify-between transition-colors"
                  >
                    <div>
                      <div className="text-xs font-bold text-slate-800">{stu.name}</div>
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 bg-white hover:bg-brand-600 hover:text-white border border-slate-200 hover:border-transparent text-slate-700 text-xs font-bold rounded-lg transition-all shadow-2xs"
                    >
                      Invite
                    </button>
                  </div>
